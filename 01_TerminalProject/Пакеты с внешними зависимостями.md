...
`rooter`
```console
[root@localhost .in]# rpm -i /usr/src/RPM/RPMS/x86_64/ncursesw-1.0-alt1.x86_64.rpm ncursesw-1.0-alt1.x86_64.rpm
...
Во время разработки окружение hasher должно соделжать сборочные зависимости. Для эксплуатации пакета достаточно уствновить только эксплуатационные. Эксплуатационные зависимсоти можно указывать явно, но в большитнстве случаев они добавляются в `.rpm`-пакет автоматически в процессе сборки. Скопируем наш пакет из `hasher` и попробуем установить его в чистое окружение без предварительной установки `ncurses`. 

**TODO** Показать, где в логе сборки запускается поиск requres

**TODO** rpmquery --provides ncursesw-1.0-alt1.x86_64.rpm
...
%install
install -D 755 %_builddir/%name-%version/%name %{buildroot}%{_bindir}/%name

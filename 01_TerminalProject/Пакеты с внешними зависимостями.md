...
Поскольку все эксплуатационные зависимости были указаны ещё при сборке пакета, при установке в чистое окружение они также устанавливаются без дополнительных действий:
...
При создании окружения вокруг пакета запускается автоматическая сборка, а также проводится проверка соответствия сборки пакетов правилам `Sysiphus` с помощью встроенной в `hasher` утилиты `sysiphus_check`. По правилам `Sysiphus` пакет должен принадлежать разработчику `<User@altlinux.org>`, все комментарии в директиве `%changelog` должны также принадлежать только таким пользователем. При создании окружения также и текущий разработчик должен соответствовать формату. 

...
Во время разработки окружение hasher должно соделжать сборочные зависимости. Для эксплуатации пакета достаточно установить только эксплуатационные. Эксплуатационные зависимости можно указывать явно, но в большинстве случаев они добавляются в `.rpm`-пакет автоматически в процессе сборки. Скопируем наш пакет из `hasher` и попробуем установить его в чистое окружение без предварительной установки `ncurses`. 

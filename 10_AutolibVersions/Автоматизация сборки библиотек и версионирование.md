...
При работе с проектами частой проблемой становится подключение библиотек. Само создание библиотек требует отдельной компиляции и компоновки её исходников (что вручную занимает много сил и времени), а в случае динамических библиотек также в обязательном порядке необходимо вручную указывать места их расположения через `LD_PRELOAD` и `LD_LIBRARY_PATH`. Для удобной работы с библиотеками и их тестированию необходимо использовать специальные инструменты по автосборке. Классическим инструментом сборки является [`libtool`](https://www.gnu.org/software/libtool/manual/libtool.html).

## Использование `libtool` вручную

Рассмотрим простую программу по инкрементированию числа, разобьём её на основной код, файл с основной функцией, файл с глобальной переменной инкрементирования и заголовочный файл:
...
```
 ...
 + Так как `libtool` - стандартная утилита работы с библиотеками, в `make` для неё существует своя переменная окружения `LTFLAGS`. При этом базовые флаги для работы с библиотеками (например, `-fPIC`) добавляются автоматически, Вручную необходимо указать только специализированные, как, например, информацию о компиляторе;
 ...
 + На основе `lo`-файлов создаётся `libtool archive` (`.la`). , являющийся основой сразу для статической и динамической библиотек. Для создания архива указывается режим компоновки `link`, а также стандартное место расположения библиотек (`-rpath`), куда будет помещены готовые библиотеки
 + Для создания бинарного файла также в режиме компоновки объединяются архив с о-файлом основной программы. Это даёт возможность убрать явное указание места загрузки динамической библиотеки при вызове функции (в частности, в рецепте `check`) — `libtool` автоматически определяет расположение библиотеки и занимается её подключением.
...
Во-первых, среди генератов появились два файла `inc`. Находящийся в `.libs/` — непосредственно исполняемый файл. Находящийся в текущем каталоге — на самом деле `shell`-сценарий, в котором автоматически прописываются пути к динамическим библиотекам, и программа вызывается с их неявным указыванием.

`@user`
```console
[user@VM inc-pkg]$ diff inc .libs/inc
Двоичные файлы inc и .libs/inc различаются
[user@VM inc-pkg]$ cat inc | head -n1
#! /bin/sh
[user@VM inc-pkg]$ cat inc | grep "LD_LIBRARY_PATH"
...
Во-первых, это формат хранения библиотек с сопутствующими ссылками. Бесчисловая версия используется для создания зависимостей на библиотеку при работе зависящих утилит, мажорная («основная», с одним числом) используется для указания на основную поддерживаемую версию, а последняя (единственная реальная библиотека) является самим рабочим объектом.

Во-вторых, это параметры, по которым ведётся отслеживание изменений, и правила версионирования. В случае с библиотеками версионирование описывается через изменение `ABI` - `Application Binary Interface`. Он задаёт низкоуровневые аспекты работы ПО на уровне машинного кода и инструкций процессора, и согласно нему нумерация библиотек определяется полями `current:revision:age`:
...
Сборка уже работает корректно, однако получился просто многофайловый проект. Добавим обработку `libtool`: в `configure.ac` добавим инициализацию `libtool`и укажем явный запрет сборки статической библиотеки, в `src/Makefile.am` укажем сборку библиотеки из исходников, а также связь библиотеки с исполняемым файлом.
...
Отметим, что поскольку `autotools` автоматически определяет места для установки всех компонентов проекта, библиотека `libinc.so` автоматически устанавливается в `/usr/lib64`, благодаря чему программа не требует дополнительного указания `LD_LIBRARY_PATH` . При описании файлов пакета (директива `%files`) просто указывается директория `%_libdir`.

...
Поскольку сборка пакета с исполняемыми файлами, скомпилированными для запуска `gcov`, не пригодны для пакетирования, необходимо проводить тестирование на независимой сборке. Она будет проводиться в отдельной вспомогательной директории, а запуск проверки будет контролироваться с помощью макросов `%def_enable` для отслеживания ключа запуска тестирования и `%if_enabled / %endif` для обработки случая тестирования.

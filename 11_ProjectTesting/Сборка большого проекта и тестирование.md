...
Сами тесты описываются в виде функций внутри специальных макросов `START_TEST`/`END_TEST`. Для выполнения тестов необходимо создать объекты для описания тестовых случаев и комплектов. Для каждого тестового случая создаётся свой уникальный раннер, который будет выполнять тесты. Регистрация комплектов в случаи, а тестов в комплекты — тоже задача разработчика. По выполнении всех тестов раннер необходимо вручную очистить.

Ручное составление тестовых файлов — довольно монотонная и долгая задача. Однако большая часть тестового файла вполне может быть сгенерирована, для этого в пакет libcheck входит утилита **checkmk**. Поэтому тестовые файлы пишутся всё так же на Си, но общая часть задаётся директивами, похожими на Си-препроцессор, а в `Makefile.am` добавляется вызов checkmk.
...
```
 ...
 + Для ручного сбора статистики по тестированию добавлен рецепт `checklog`
 + Непосредственно запуск утилиты gcov из основного `Makefile.am` перенаправляет вызов из `src/Makefile.am` {to: FrBrGeorge, [Я же правильно интерпретирую эту строку?]}
...
Также заметим, что для работы утилиты необходимо проводить компиляцию исходников со специальными ключами, что, в общем случае, необязательно. Вследствие этого в `configure.ac` необходимо добавить отдельный ключ для включения этой функции при тестировании.

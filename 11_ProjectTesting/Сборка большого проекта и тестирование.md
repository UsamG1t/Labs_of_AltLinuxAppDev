...
**TODO** подумать, не перевести ли термины на русский; английские термины дать олько один раз
* раннер (оставить макаронический термин)
* тестовый комплект
* тестовый случай
* тест
* фикстура (оставить макаронический термин)

**TODO** Другой вариант — взять из [https://ru.wikipedia.org/wiki:XUnit](%D0%92%D0%B8%D0%BA%D0%B8%D0%BF%D0%B5%D0%B4%D0%B8%D0%B8) (сослаться на неё точно надо)
...
В рамках Unit-тестов определён test **runner**» — основная программа, запускающая тесты. Он отвечает за проверку подключаемости тестирующего кода , фильтрацию тестов, сбор информации по запускам и т.д. Все тесты (**test**) объединены в отдельные тестирующие блоки (test **case**), проверяющие какое-то конкретное свойство проекта. Множества блоков, описывающих один раздел работы с проектом объединены в test **suite**. Как правило, в один suite входят тесты, работающие в одинаковых или похожих условиях, а если условия существенно разные (например, бэкенд в виде файла и бэкенд в базе данных) — в отдельные.
	...
	Для более точного понимания можно запомнить, что suit показывает, «в каких условиях сломалось?», case — «какая функциональность сломалась?", test — «что именно сломалось?»
...
**TODO** Минимальный пример тестового файла без `checkmk`

Большая часть тестового файла вполне может быть сгенерирована, для этого в пакет `libcheck` входит утилита `checkmk`. Тестовые файлы пишутся всё так же на Си, но общая часть задаётся директивами, похожими на Си-препроцессор.
...
Традиционно исходники для `checmk` имеют расширение `.ts` (от **t**est **s**uite), поэтому многие текстовые редакторы пытаются включить подсветку синтаксиса TypeScript — она, конечно, плохо подходит.
...
**TODO** кратко описать, что задано в этом файле ↑
...
**TODO** дописать текст между примерамиж; где `1.1-alt1` превращается `1.2-alt1`?

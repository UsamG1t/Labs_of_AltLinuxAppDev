...
Скомпилируем программу и проверим её работу. Заметим, что макросы `PACKAGE` и `LOCALE_PATH` в самой программе не попределены — мы можем задать их из командной строки как нам удобнее. В нашемм случае переводы искать надо прямо в текущем каталоге, а домен называетя `sheepcounter`:
...
Для сборки напишем небольшой `Makefile`, в котором макрос `PACKAGE` зададим сразу, а вот расположение директории с локалями оставим на усмотрение автора _пакета_.
...
CFLAGS = -Wall -O0 -g -DPACKAGE='"$(PROGRAM)"'
CC = cc
...
В файл спецификации добавим определение недостатощего макроса `LOCALE_PATH` таким образом, чтобы перевод находился _после установки_ пакета.
...
Классическая структура autotools предполагает отдельный `Makefile` в каждом компоненте проекта — в базовой дректории, в `src/` и в `/po`; для двух из них напишем примитивные `Makefile.am`.
...
Добавим `config.h` в список заголовчных файлов нашей программы:
...
Отредактируем `spec`-файл (теперь нём нет необходимости доопределять какие-либо парамретры) и соберём пакет:

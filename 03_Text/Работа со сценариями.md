...
+ Для UI `dialog` использует `ncurses`, так что весь _управляющий_ вывод (например, метку выбранного пункта меню) выводится в другой дескриптор (2, то есть `stderr`) и пренаправляется в файл в помощью
+ Файл этот — временный, заводится при старте с помощью утилиты `mktemp`; она же обкспечивает ему уникальность имени.

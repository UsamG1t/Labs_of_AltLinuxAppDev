...
Базовый набор команд Unix Shell и их применение описаны [в главе](https://github.com/UsamG1t/Methodics_of_LinuxAppDev/blob/master/Methodical_manual/03_Text/3.%20%D0%9E%D0%B1%D1%80%D0%B0%D0%B1%D0%BE%D1%82%D0%BA%D0%B0%20%D1%82%D0%B5%D0%BA%D1%81%D1%82%D0%BE%D0%B2%20%D0%B8%20%D1%81%D1%86%D0%B5%D0%BD%D0%B0%D1%80%D0%B8%D0%B8.md#unix-shell). Соберём пакет с приложением, написанном на языке склейки.

`@builder`: `RPM/SOURCES/todo-pkg-1.0.sh`
...
+ Для UI dialog использует Ncurses, так что весь _управляющий_ вывод (например, метка выбранного пункта меню) выводится в другой дескриптор (2, то есть stderr) и перенаправляется в файл
...
`@rooter`
```console
[root@localhost .in]# rpm -i /usr/src/RPM/RPMS/x86_64/todo-pkg-1.0-alt1.x86_64.rpm
error: Failed dependencies:
       dialog is needed by todo-pkg-1.0-alt1.x86_64
[root@localhost .in]# rpmquery --requires --package /usr/src/RPM/RPMS/x86_64/todo-pkg-1.0-alt1.x86_64.rpm
/bin/bash
/etc/bashrc
coreutils
dialog

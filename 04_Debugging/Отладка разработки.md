...
Постоянная настройка `hasher` не очень удобна в процессе интенсивной работы. Поэтому мы предлагаем использовать `bash`-сценарий для быстрой настройки рабочего окружения.

Данный сценарий позволяет воспользоваться `hsh-shell`:
...
**TODO** Всё как обычно ☹ — я хочу переписать это (хотя сразу выглядит лучше!). Оформите в виде отдельного файла, пожалуйста.

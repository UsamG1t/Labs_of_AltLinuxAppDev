...
1. Компиляция программы с нулевой оптимизацией. (флаг `-O0`) для избежания несостыковок исходного текста программы с бинарным файлом (н-р, в исходнике есть переменная, которая нигде не используется. При оптимизации переменная будет удалена, а в исходниках её упоминание останется);
...
3. Доступ к исходникам, на которые будет осуществляться ссылка (при этом они не должны считаться временными файлами для системы, при перенесении исходников в `/tmp` ничего не заработает). Отлаживать возможно и без исходников, но в формате только ассемблерного кода.

## `GDB`

Вообще, отладка - это огромная тема, подробнее этот аспект можно изучить в [этой](https://en.wikibooks.org/wiki/Linux_Applications_Debugging_Techniques) книжке. Рассмотрим более подробно отладчик `gdb - GNU Debugger`.
...
```
 ...
 + Перезапустить сервис `hasher-privd` (***TODO***: вот [здесь](https://www.altlinux.org/Hasher/gdb) этого не сказано. А чем очевиднее действие, тем чаще и громче о нём надо рассказывать)
```console
...
(gdb) break fun2 # Установка точки остановы на объекте  
Breakpoint 1 at 0x1140: file ex.c, line 5.  
(gdb) break 9 # Установка точки остановы на конкретной строке кода
...
9               c += 1;  
(gdb) print c # Просмотр значения переменной в точке остановы
$1 = 42  
(gdb) next
...
5               return n*2+1;  
1: n = 86  
(gdb) list  # Просмотр блока кода вокруг точки останова
1       #include <stdio.h>  
2       int N = 42;  
3  
4       int fun2(int n) {  
...
В `spec`-файле появилась новая директива `%check`. Указанные в `%check` команды обязаны завершаться с кодом возврата 0.

Проверка осуществляется при сборке пакета:

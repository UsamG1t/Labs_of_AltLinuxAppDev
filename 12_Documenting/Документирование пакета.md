...
За пределами внутреннего документированеия остаётся всё остальное информацонное пространство
 ...
 + Т. н. «сайтогенераторы» — приложения, предназначенные для порождения статического HTML и последующей публикации. Это могут быть специализированные приложения, наподобие [Pelican](https://blog.getpelican.com/) или [Jekyll](https://jekyllrb.com/), но можно заставить работать в таком режиме и саму систему технического докуметирования.
 + Специализированные сервисы документирования ([ReadTheDocs](https://readthedocs.com/), [Git Book](https://www.gitbook.com/))

## Doxygen

Самым популярным на сегодня инструментом документирования является [Sphinx](https://www.sphinx-doc.org/); в нашем проекте косвенно задействован другой — [Perl Podlators](https://metacpan.org/dist/podlators) — с ео помощью форматируется man-страница. Внутренне документирование мы организуем с помощью ещё одной классической системы — [Doxygen](https://www.doxygen.nl/). Процесс создания документации с её помощью поддерживается autotools, при этом поддерживается немалое количество _выходных форматов_ доуцментации — от HTML-страницы до печатного варианта.
...
Само документирование представляет собой комментирование исходников, которое и уйдёт в документацию. Doxygen поддерживает как однострочные, так и многострочные комментарии, при чём вне зависимости от формата документирования в разных языках программирования.

**TODO** Далее приведён пример _недокументированного_ текста: комментарии в нём не распознаются Doxygen (см. пакет)

**TODO** В документиции _нужно_ кратко сказать, что функция делает, и описать параметры функции и возвращаемое значение, иначе вообще зачем она. Примеры тут: https://git.sr.ht/~frbrgeorge/namegen/tree/master/item/src/libnamegen.c
...
**TODO** После актуального документирования посмотреть, что именно генерируется в этих man-ах, сколько их, и которые из них вообще имеют смысл. Вангую, что именно `syscall(3)` — не имеет. Возможно, никакие не имеют.
...
**TODO** Титульная страница сейчас тоже пустая. Туда надо что-то дописать, и заодно намекнуть, что в принципе Doxygen можно использовать для созджания _любой_ документации

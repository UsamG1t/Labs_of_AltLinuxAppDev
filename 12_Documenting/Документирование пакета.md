...
При разработке продукта одной из ключевых составляющих является поддержка **информационного пространства**, позволяющего пользователям ориентироваться в нём и разбираться с доступным функционалом. Информационное пространство состоит как из описания внутренних элементов продукта, так и из информации о возможностях самого продукта и о правилах взаимодействия с ним. При поддержке продукта основную часть информационного пространства заполняет описание и струтуризация всех проводящихся изменений проекта.

В данной главе мы остановимся на документировании исходников. Существует несколько методик документирования:
 + Комментарии (одним из ярких примеров является методика  [Literate Programming](https://ru.wikipedia.org/wiki/Грамотное_программирование), предложенная Кнутом);
 + Документирование `API` за счёт внутренних особенностей языка (как, например, «повисшие строки» в Python);
 + Документирование с использованием систем синтаксического разбора (как, например, [Sphinx](https://www.sphinx-doc.org/)):

В собираемом проекте уже используется одна из таких систем — [Perl Podlators](https://metacpan.org/dist/podlators) для сборки man-страницы документации

Формат документирования также может быть различным:
 + Offline-справочники ([help](https://www.opennet.ru/man.shtml?topic=help&category=1&russian=4) и [man](https://man7.org/linux/man-pages/man1/man.1.html))
 + Online-документация (wiki, [Pelican](https://blog.getpelican.com/))
 + Специализированные сервисы документирования ([`ReadTheDocs`](https://readthedocs.com/), [`Git Book`](https://www.gitbook.com/))

## Doxygen

Будем рассматривать одну из классических систем синтаксического разбора [Doxygen](https://www.doxygen.nl/). Процесс создания документации с его помощью поддерживается autotools, при этом сам документатор позволяет собирать огромное количество вариантов документации.
...
Файл представляет из себя огромный настройщик с описанием параметров:
...
Опишем основные параметры настройки документации:
...
Само документирование представляет из себя комментирование исходников, которое и уйдёт в документацию. Doxygen поддерживает как однострочные, так и многострочные комментарии, при чём вне зависимости от формата документирования в разных языках программирования.
...
/*
* Binary search of executing syscall
*/
...
В `configure.ac` опишем использование Doxygen:
...
DX_INIT_DOXYGEN([syscall], [Doxyfile], [doc/doxygen])
...
Для генерации дополнительных справочников, например, man, необходимо указать это в настройщике, а также добавить правила генерации в Makefile.am

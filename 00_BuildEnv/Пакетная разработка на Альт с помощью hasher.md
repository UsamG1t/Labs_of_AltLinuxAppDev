...
При регистрации создаются два дополнительных пользователя. `B`-пользователь отвечает за непосредственно сборку пакета, `A`-пользователь выступает в виде  `fakeroot`, и выполняет роль суперпользователя в рамках системы сборки.
...
Рассмотрим [вывод](obsidian://open?vault=Labs_of_AltLinuxAppDev&file=00_BuildEnv%2FAttached_materials%2F%D0%A1%D0%BE%D0%BA%D1%80%D0%B0%D1%89%D1%91%D0%BD%D0%BD%D1%8B%D0%B9%20%D0%B2%D1%8B%D0%B2%D0%BE%D0%B4%20%D0%BF%D1%80%D0%B8%20%D1%81%D0%BE%D0%B7%D0%B4%D0%B0%D0%BD%D0%B8%D0%B8%20%D0%BE%D0%BA%D1%80%D1%83%D0%B6%D0%B5%D0%BD%D0%B8%D1%8F%20hasher) при создании окружения и обсудим некоторые составные части `hasher`. \
...
Внутри `hasher` основной пользователь и два дополнительных обретают описанные выше характеристики: основной пользователь - `caller` - имеет право лишь просматривать данные и получать доступ к готовым пакетам, `A`-пользователь - `rooter` - выполняет действия суперпользователя в рамках инструмента, `B`-пользователь - `builder` - осуществляет сборку пакетов. \

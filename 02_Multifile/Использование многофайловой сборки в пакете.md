...
install -D -pm 755 libdynlib.so %buildroot%_libdir/libdynlib.so     
    
       
%files        
%_bindir/*        
...
`@builder`
```console
[builder@localhost Multilab-1.0]$ ldd Multilab-a # Только стандартные библиотеки 
...
[builder@localhost Multilab-1.0]$ ldd Multilab-so # Нужна дополнительная динамическая, но автоматически она не находится
...
[builder@localhost Multilab-1.0]$ LD_LIBRARY_PATH=`pwd` ldd Multilab-so # После явного указания расположения библиотека нашлась  

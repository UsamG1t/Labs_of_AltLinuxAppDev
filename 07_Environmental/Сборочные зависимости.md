...
**TODO** Добавить в тот же printw вывод версии
...
**TODO** везде заменить `install` на `%makeinstall_std`
...
Подробнее остановимся на самом исполнении `configure` при сборке. В макроче к нему добавляяется множество флагов сборки, указывающих на местоположение исходников, библиотек и т.д.
...
Значение макроса RPM выставляется в spec-файле, но может быть переопределено из командной строки при запуске, например, `rpm … --disable=keypad …`. 

Добавим возможность включения и выключения режима распознавания функциональных клавиш при сборке.

**TODO** ↑ дописать пример

Второй параметр сборки — frame — требует задавать некоторое значение. Определим это значение заранее в spec-файле, например, `%define framewidth 3`, и явно укажем его в качестве параметра: `configure … --enable-frame=%framewidth`. Тогда при вызове `rpm` можно будет переопределить макрос с помощью, допустим, `rpm -D "framewidth 5" …`.
...
**TODO** раз и навсегда присвоить этому сценарию имя (возможно, в той лабе, где он появляется), там же написать внятно и заметно, что именно он делает, и здесь ещё раз коротко пояснить.
...
**TODO** это лишнее, см. другой TODO
...
**TODO** Вот это какая-то фигня, так точно не должно получаться ↓
```
├── autoenv-pkg-1.1
│   ├── Makefile.am
│   ├── configure.ac
...
**TODO** Это только всех запутает, мне кажется, достатояно показать, как они прямо в hasher работают в разных вариантах. Ну то есть буквально собрать по дефолту, собрать с отключённым keypad, собрать с конской рамкой.
